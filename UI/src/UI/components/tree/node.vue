    ...
    <collapse-transition :appear="appear">
        <ul :class="classes">
            <li :class='{border:showArrow}'>
                ...
                <span v-else :class="titleClasses" @click="handleSelect"><Icon v-if="!data.expand && !showArrow" type="iconli" class="ivu-tree-iconli"/><span>{{ data.title }}</span></span>
                <Tree-node
                        v-if="data.expand"
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            arrowType () {
                let type = 'icon-tree-top-jia';//修改图标
                if (this.$IVIEW) {
                ...
                // 树展开，收起图标更改，以前用的一个图标，通过旋转来改变的
                if (this.data.expand) {
                    type = 'icon-tree-top-jian'
                }
                return type;
            },
